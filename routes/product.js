...
router.get('/get',auth.authenticateToken,(req,res,next)=>{
    var query = "select p.id,p.name,p.description,p.price,p.status,c.id as catergoryId,c.name as catgoryName from product as p INNER JOIN category as c where p.categoryId = c.id";
...
router.delete("/delete",auth.authenticateToken,checkRole.checkRole,(req,res,next)=>{
    const id = req.params.id;
    var query = "delete from prodect where id=?";
    connection.query(query,[id],(err,results)=>{
        if(!err){
            if(results.affectRows == 0){
                return res.status(404).json({message: "Product Id was not found"});
